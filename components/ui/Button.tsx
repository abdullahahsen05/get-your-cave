...
export function GhostButton({ className = "", children, ...props }: Props) {
  return (
    <button className={`inline-flex min-h-11 items-center justify-center whitespace-nowrap rounded-full px-4 py-3 text-sm font-semibold text-primary transition-colors hover:bg-secondary-container sm:px-6 ${className}`} {...props}>
      {children}
    </button>
  );
}
